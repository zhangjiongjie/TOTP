...
- WebDAV 自动同步、冷启动 key cache、账号级合并和冲突提示。
...
- 普通主密码解锁：用主密码 + salt 重新执行 PBKDF2，解密本地 vault。
...
WebDAV 远端 vault 使用和导入导出一致的加密规格：
...
Android 端额外增加了 WebDAV remote key cache：

- 进程内缓存：同一进程内重复同步不再重复 PBKDF2。
- 持久缓存：remote key 使用 Android Keystore AES-GCM 加密后写入 SharedPreferences。
- 懒加载：持久缓存只在 WebDAV 加密或解密时读取，不在 App 启动时主动读取。
- 失效条件：WebDAV server、路径、用户名、密码、salt、iterations 或 passwordVerifier 不匹配时不会命中。

这个缓存不单独要求系统凭据校验，安全边界依赖 App 已解锁态、Android 应用沙箱和 Keystore non-exportable key。这样可以减少同步时频繁弹系统凭据，优先保证同步性能和体验。
...
- 导入 / 导出没有复用 WebDAV remote key cache，每次会重新执行备份 KDF。
