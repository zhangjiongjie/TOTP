...
`apps/harmony-app` 是 TOTP 多端工程中的 HarmonyOS NEXT 客户端，面向手机和平板设备。当前版本已经从最小骨架演进为可日常使用的完整客户端，支持本地加密保管库、TOTP 验证码展示、WebDAV 同步、导入导出、修改主密码和生物识别解锁。
...
| 共享策略 | 当前为 ArkTS 原生实现；协议、备份格式和同步语义与 Android、浏览器插件端对齐 |
...
- WebDAV 关闭后，首页和设置页提示会回到「WebDAV 同步未开启，本地模式。」。
...
- 导入 v2 加密备份时，备份主密码必须能解开该备份；重新开启 WebDAV 后会优先下载远端并采用远端 `vaultKey`。
...
| 远端主密码已变化 | 阻止同步，验证远端主密码后继续同步 |
| WebDAV 关闭 | 清除远端阻塞提示，回到本地模式 |
...
HarmonyOS 客户端目前没有直接依赖这些 TypeScript 共享包，而是在 ArkTS 侧实现等价逻辑。这样可以降低首个版本的构建和运行风险。涉及 v2 envelope、WebDAV payload、账号级合并、远端主密码阻塞和导入导出格式的变更，需要同步对照 Android 与浏览器插件端行为，避免跨端语义漂移。
...
- 可以继续补充端到端测试夹具，覆盖 Android、浏览器插件端与 HarmonyOS 端的导入导出、加密备份和 WebDAV 同步一致性。
