...
这是一个本地优先的 TOTP 身份验证器工程，当前支持 Android 原生应用、HarmonyOS NEXT 应用和 Chrome / Edge 浏览器插件。工程采用 monorepo 组织方式，把 TOTP、加密保管库、WebDAV 同步、导入导出、品牌图标匹配等通用能力沉到共享包中，各端只保留平台相关的 UI、存储和系统能力适配。
...
- **跨端同步：** 支持 WebDAV 同步、远端初始化、远端 vault key 权威源、账号级合并和冲突处理；远端 WebDAV `encryptedVault` 使用同一套 v2 envelope。
- **迁移工具：** App 内不再兼容旧 v1 加密数据，旧加密导出可通过 `tools/migrate-vault-v1-to-v2.mjs` 转换为 v2 导出后再导入。
- **解锁体验：** Android 端支持系统凭据 / 生物识别快速解锁；HarmonyOS 端支持生物识别；浏览器插件端支持 WebAuthn / Windows Hello 解锁。
- **品牌图标：** 品牌匹配规则和图标资源集中维护，浏览器直接读取共享包，HarmonyOS 通过脚本同步资源与匹配代码。
- **暗色模式：** 三端界面按当前设计适配浅色 / 深色模式。

## v2 数据模型

当前三端统一使用 v2 key envelope。账号数据不再直接由主密码派生 key 加密，而是在创建保管库时生成随机长期 `vaultKey`：
...
- `vaultKey` 创建后长期稳定，用于加密账号数据。
- 主密码只负责派生 `wrappingKey`，再加密 / 解密 `vaultKey`。
- 修改主密码只重新包裹 `vaultKey`，不会重加密账号数据。
- WebDAV 开启后，远端 vault key 是权威源；客户端接入远端、自动合并或验证远端密码成功后，本地会采用远端 `vaultKey`。
- 如果远端主密码已被其他端修改，本端会进入 `BlockedRemotePassword` 状态，必须先验证远端主密码，验证成功后再按正常同步逻辑做账号级合并。
...
- Node.js 20 或更高版本。
- npm workspace。
- Android Studio 和 Android SDK，用于 Android 端构建和真机调试。
...
Android、HarmonyOS 和浏览器插件端使用相同的数据语义，导入后会写入本地保管库，并按当前 WebDAV 配置触发同步。
...
- 涉及 UI 的变更，应分别在 Android 真机、HarmonyOS 真机 / 模拟器和浏览器插件上验证。
